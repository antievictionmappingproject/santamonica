...
Date of Eviction        ---> last_transaction            --->
# of Units Evicted      ---> bedrooms --->

Landlord                ---> owner

// Hand entered Map + Animation varibales
lat, long               ---> [37.8160921,-122.2516301]    --->
...
$( document ).ready(function() {
  ...
  sql.execute("SELECT the_geom, address, cartodb_id, owner, last_transaction, bedrooms FROM {{table_name}} WHERE NOT(the_geom IS NULL OR last_transaction IS NULL OR bedrooms IS NULL) ORDER BY last_transaction ASC", {
    table_name: 'foreclosure'})
  .done(function(collection) {

    /* Set animation variables */
    var cumEvictions = 0;
    var firstDate = new Date(collection.features[0].properties.last_transaction);
    ...
    maxTime =  Date.parse(collection.features[(collection.features.length)-1].properties.last_transaction)+4000000;
    step = Math.floor((maxTime - startingTime) / (animationDuration * (1000 / animationInterval)));

    collection.features.forEach(function (d) {
      d.LatLng = new L.LatLng(d.geometry.coordinates[1],d.geometry.coordinates[0]);
      cumEvictions += d.properties.bedrooms;
    ...
    // .attr("r", function(d) { return ((d.properties.bedrooms / (2 * Math.PI)) * finalRadiusMultiplier);})
    // .attr("r", function(d) { return (d.properties.bedrooms / 2) * finalRadiusMultiplier;})
    // .attr("r", function(d) { return (Math.pow((d.properties.bedrooms / 2), 0.9) * finalRadiusMultiplier) + 1.5 ;})
    .attr("r", function(d) { return (d.properties.bedrooms / 2 * finalRadiusMultiplier) + 1.5 ;})
    ...
    node.on("mouseover", function (d) {
      var fullDate = d.properties.last_transaction;
      ...
      var owner = d.properties.owner;
      var bedrooms = d.properties.bedrooms;
      var addressText = "";
      var ownerText = "";

      var bedroomText = bedrooms + " eviction" + "<br>";
      if (bedrooms > 1) { unitText = bedrooms + " evictions" + "<br>"; }
    ...
    function filterCurrentPoints(){
       ...
       .filter(function(d) { return Date.parse(d.properties.last_transaction) < counterTime;}) 
       .attr("visibility", "visible");

      /*Animate most recent evictions*/
      filtered.filter(function(d) {
        return Date.parse(d.properties.last_transaction) > counterTime-step;
      }) 
      .append("circle")
      .attr("r", 4)
